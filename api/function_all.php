...
function checkWebPermission($conn, $filename)
{
    // echo 'filename:' . $filename;
    
    $res = 0;

    // echo 'username:' . $_SESSION["username"] . ' usertype:' . $_SESSION["usertype_id"];
    // exit;
    // Check if $_SESSION["userID"] and $_SESSION["userTypeID"] are set
    if (isset($_SESSION["username"]) && isset($_SESSION["usertype_id"])) {
        // $userID = $_SESSION["userID"];
        $userTypeID = $_SESSION["usertype_id"];
        // echo 'userTypeID:' . $userTypeID . ' filename:' . $filename;
        // exit;
        // Prepare the SQL statement
        $stmt = $conn->prepare("SELECT * FROM `permission_web_usertype_full` WHERE `usertype_id` = ? AND `file_name` = ?");
