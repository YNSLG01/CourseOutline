...
if (file_exists('../conn.php')) {
    ...
    if (isset($_GET['department_id']) && is_numeric($_GET['department_id'])) {
        ...
        $stmt = $conn->prepare("SELECT `department_id`, `d_name` FROM `satit`.`department` WHERE `department_id` = ?");
        $stmt->bind_param("i", $department_id);
    } else {
        // Prepare the SQL statement without WHERE clause
        $stmt = $conn->prepare("SELECT `department_id`, `d_name` FROM `satit`.`department`");
    }
